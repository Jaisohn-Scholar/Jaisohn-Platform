...
export default function OpportunitiesPage() {
  ...
  const getApplyButton = (opp: Opportunity) => {
    ...
    if (isFilled) {
      return (
        <span className="inline-block text-xs font-medium px-3 py-1 rounded-md bg-gray-100 text-gray-400 mt-auto cursor-default select-none">
          All Spots Filled
        </span>
      );
    }
    if (!session) {
      return (
        <Link
          href="/login"
          className="inline-block text-sm font-medium px-4 py-1.5 rounded-md border border-[#101661] text-[#101661] hover:bg-[#101661] hover:text-white transition-colors mt-auto"
        >
          Sign In to Apply
        </Link>
      );
